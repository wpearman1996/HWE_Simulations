...
file_gl$pop<-as.factor(pop_list)
pops <- as.character(unique(file_gl$pop))

hw_tests <- list()
for (i in 1:length(pops)){
  hw_tests[[i]]<-check_hw_per_pop(file_gl,pops[i])
}
common_values = Reduce(intersect, hw_tests) # Identify loci out of HWE in all populations
any_hwe <- unique(unlist(hw_tests)) # Identify loci out of HWE in at least one population
hwe_across <- check_hw_per_pop_across(file_gl)
writeLines(common_values,paste0(opt$dir,opt$pre,"hwe_out_all.txt"))
writeLines(any_hwe,paste0(opt$dir,opt$pre,"hwe_out_any.txt"))
writeLines(hwe_across,paste0(opt$dir,opt$pre,"hwe_out_across.txt"))
