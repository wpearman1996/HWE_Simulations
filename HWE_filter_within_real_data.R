...
option_list = list(
  ...
  make_option(c("-m","--pop"),type="character", default=NULL,
              help="output prefixe name [default= %default]", metavar="character")
); 
...
if (is.null(opt$pop)){
  print_help(opt_parser)
  stop("Popmap is required", call.=FALSE)
}
if (is.null(opt$pre)){
  print_help(opt_parser)
...
filter_vcf_within<-function(vcf,hwe_list,pop_list){
  subd_vcf<-read.vcfR(vcf)
  rownames(subd_vcf@gt)<-subd_vcf@fix[,3]
  sort.df <- with(pop_list,  pop_list[order(pop_list$V2) , ])
  pop_list<-sort.df$V2
  pop_list_rep<-c("x",pop_list)
  pop_list_df<-data.frame(1:length(pop_list_rep),pop_list_rep)
  colnames(pop_list_df)<-c("Num","Pop")
  gt_replace<-subd_vcf@gt[,2:length(colnames(subd_vcf@gt))][,sort.df$V1]
  gt_replace<-cbind(subd_vcf@gt[,1],gt_replace)
  colnames(gt_replace)<-c("FORMAT",sort.df$V1)
  subd_vcf@gt<-gt_replace
  for (Q in 1:length(unique(pop_list))){
    subd_vcf@gt[rownames(subd_vcf@gt) %in% hwe_list[[Q]],pop_list_df$Num[pop_list_df$Pop==unique(pop_list)[Q]]]<-NA
  }
  common_values = Reduce(intersect, hw_within)
  subd_vcf@gt<-(subd_vcf@gt[!rownames(subd_vcf@gt) %in% common_values,])
  subd_vcf@fix<-(subd_vcf@fix[!subd_vcf@fix[,3] %in% common_values,])
  subd_vcf
}

setwd("/mnt/9edae943-386a-4604-bb58-13212b0a3287/William/simulations_revisions/real_data_within/")

file <- read.vcfR("./furseal/populations.snps.vcf",verbose = FALSE) 
file_gl <- vcfR2genlight(file)

pop_list<-read.table("./furseal/popmap.tsv")
file_gl$pop<-as.factor(pop_list$V2[match(pop_list$V1,file_gl$ind.names)])
pops <- as.character(unique(pop_list$V2))

hw_within <- list()
for (i in 1:length(pops)){
  hw_within[[i]]<-check_hw_per_pop(file_gl,pops[i])
}
common_values = Reduce(intersect, hw_within) # Identify loci out of HWE in all populations

filtered_within<-filter_vcf_within("./furseal/populations.snps.vcf",hw_within,pop_list)#opt$vcf)

write.vcf(filtered_within,file=paste0("./furseal/","furseal","hwe_out_within.vcf.gz"))

#Zebra
file <- read.vcfR("./zebra/populations.snps.vcf",verbose = FALSE) 
file_gl <- vcfR2genlight(file)

pop_list<-read.table("./zebra/zebra_popmap.txt")
file_gl$pop<-as.factor(pop_list$V2[match(pop_list$V1,file_gl$ind.names)])
pops <- as.character(unique(pop_list$V2))
...
filtered_within<-filter_vcf_within("./zebra/populations.snps.vcf",hw_within,pop_list)#opt$vcf)

write.vcf(filtered_within,file=paste0("./zebra/","zebra","hwe_out_within.vcf.gz"))

#isopod
file <- read.vcfR("~/HWE_Simulations/real_data/isopods/vcfs/VCFS_r2/monomorph_fix/ploidy_HWE_nloci_testing/isopod_genfilt.vcf",verbose = FALSE) 
file_gl <- vcfR2genlight(file)

#pop_list<-readLines("./isopod/isopod_popmap.txt")
#pop_list<-data.frame(file_gl$ind.names,pop_list)
...
pop_list<-data.frame(file_gl$ind.names,file_gl$pop)
colnames(pop_list)<-c("V1","V2")
...
filtered_within<-filter_vcf_within("~/HWE_Simulations/real_data/isopods/vcfs/VCFS_r2/monomorph_fix/ploidy_HWE_nloci_testing/isopod_genfilt.vcf",hw_within,pop_list)#opt$vcf)

write.vcf(filtered_within,file=paste0("./isopod/","isopod","hwe_out_within.vcf.gz"))
